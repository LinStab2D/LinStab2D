function BF = example_2_readbaseflow(mesh,BF)
    ...
    BF.U(mesh.usedInd) = interp2(BFdata.X,BFdata.Y,BFdata.U_mean,X(mesh.usedInd),Y(mesh.usedInd),'linear'); 
    BF.V(mesh.usedInd) = interp2(BFdata.X,BFdata.Y,BFdata.V_mean,X(mesh.usedInd),Y(mesh.usedInd),'linear'); 
    BF.W(mesh.usedInd) = interp2(BFdata.X,BFdata.Y,BFdata.W_mean,X(mesh.usedInd),Y(mesh.usedInd),'linear'); 
    ...
    %% Apply a nan mask for better plotting.
    nanMask = nan(size(X));
    nanMask(mesh.usedInd)=1;
    
    BF.RHO      = BF.RHO.*nanMask;    
    BF.U        = BF.U  .*nanMask;    
    BF.V        = BF.V  .*nanMask;    
    BF.W        = BF.W  .*nanMask;    
    BF.T        = BF.T  .*nanMask;    
