...
% Output options
verbose = true; % Plot mesh, base flow and results
save_result = false; % Saves results as .mat file
compute_adj = false; % Computes adjoint modes

% Input file path
path_to_h5 = "Ar01_S04_PINN_1_250x250ext_Rmax5_Zmax5_baseflow_v3.h5"; % Base flow file path
...
for j=1:length(Re_list)
    for i=1:length(m_list)
        ...
        save_file_name= "Ar01_S04_250x250_m"+num2str(m_list(i))+"_Re"+num2str(Re_list(j))+"_incomp";
        
        % Physical parameters
        baseFlow.Re     = Re_list(j);          % Reynolds number
        m               = m_list(i);
        St              = 0.1;
        omega_target    = St*2*pi + 0.05i; 
        ...
        borders             = 'l';  
        vars                = 'p';
        [A,B,idx_dirchletp]   = BC_Dirichlet_incomp(A,B,idx,borders,vars);
